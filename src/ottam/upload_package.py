...
- hashtags: choose only the hashtags that genuinely help this particular topic. There is NO preferred count. Avoid filler and repeated channel-wide bundles.
...
EPISODE:\n{context}"""
    raw = client.chat_stream(model=model, messages=[{"role": "system", "content": SYSTEM}, {"role": "user", "content": prompt}], temperature=0.55, max_tokens=3500)
...
EPISODE:\n{context}"""
    raw = client.chat_stream(model=model, messages=[{"role": "system", "content": SYSTEM}, {"role": "user", "content": prompt}], temperature=0.62, max_tokens=3000)
