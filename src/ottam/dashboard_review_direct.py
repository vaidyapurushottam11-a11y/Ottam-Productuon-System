...
def is_script_review(run: dict[str, Any], episode_id: str) -> bool:
    ...
    review_done = any(step.get("name") == "Finalize · Mark script review checkpoint" and step.get("conclusion") == "success" for step in timeline)
    video_ready_done = any(step.get("name") == "Packaging · Require VIDEO_READY" and step.get("conclusion") == "success" for step in timeline)
...
def review_snapshot(episode_id: str, run: dict[str, Any]) -> dict[str, Any]:
    ...
    progress = dashboard._run_progress(run)
    progress.update({"workflow_status": "awaiting_script_approval", "current_stage": "Script ready — waiting for your approval", "stage_index": 4, "completed_stages": 4, "progress_percent": 40, "stage_elapsed_seconds": None})
    job.update({"status": "AWAITING_SCRIPT_APPROVAL", "conclusion": "success", "ready": False, "active": False, "awaiting_script_approval": True, "script": script_path.read_text(encoding="utf-8").strip() if script_path.exists() else "", "episode_state": state, "progress": progress, "can_retry": False, "build": BUILD})
...
def history_rows_direct() -> list[dict[str, Any]]:
    ...
    for run in dashboard._runs("production.yml"):
        ...
        if episode_id not in groups:
            groups[episode_id] = {"episode_id": episode_id, "title": dashboard._topic_from_run(run) or episode_id, "latest_run": run, "created_at": created}
    ...
    for episode_id in order:
        ...
        status = "IN_PROGRESS" if run_status != "completed" else "AWAITING_SCRIPT_APPROVAL" if awaiting else "COMPLETED" if conclusion == "success" else "FAILED"
        row = {**item, "status": status, "workflow_status": run_status, "conclusion": conclusion, "run_id": run.get("id"), "run_url": run.get("html_url"), "awaiting_script_approval": awaiting}
        if status == "COMPLETED":
...
def production_history_direct():
    return jsonify({"items": history_rows_direct(), "build": BUILD})


def production_history_details_direct(episode_id: str):
    run = dashboard._find_run("production.yml", f"OTTAM Production {episode_id}")
    if not run:
        return jsonify({"error": "episode not found", "build": BUILD}), 404
    if is_script_review(run, episode_id):
        payload = review_snapshot(episode_id, run)
        return jsonify({"status": "awaiting_script_approval", "episode_id": episode_id, "script": payload.get("script") or "", "message": "Script is waiting for approval before narration and visuals.", "build": BUILD})
    if run.get("status") == "completed" and run.get("conclusion") == "success":
        payload = cold._snapshot_without_blocking(episode_id, run)
        if payload.get("ready"):
            return jsonify({"status": "ready", "episode_id": episode_id, "package": payload.get("package") or {}, "video_url": f"/files/{episode_id}/video", "thumbnail_url": f"/files/{episode_id}/thumbnail", "captions_url": f"/files/{episode_id}/captions", "build": BUILD})
        return jsonify({"status": "restoring", "episode_id": episode_id, "message": (payload.get("progress") or {}).get("current_stage") or "Restoring completed assets from GitHub", "build": BUILD})
    payload = dashboard._production_snapshot(episode_id, run=run)
    return jsonify({"status": "failed" if run.get("status") == "completed" else "in_progress", "episode_id": episode_id, "failure": payload.get("failure") or {}, "run_url": run.get("html_url"), "build": BUILD})
